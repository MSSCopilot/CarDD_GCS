        ...
        for prediction2 in output2['predictions']:
            ...
            if overlap_percentage > 0.8:
                ...
                if prediction1['confidence'] > prediction2['confidence']:
                    ...
                    print(f"adding {prediction2['class']} with conf={prediction2['confidence']} instead of {prediction1['class']} with conf={prediction1['confidence']}")
                    combined_predictions.append(prediction2)
                if prediction2['class'] == 'smash':
                    print('Force adding smash')
                    combined_predictions.append(prediction2)
                    break
...
def run_model(image_path, confidence, model_number):
    ...
    project = rf.workspace().project("carddseg") if model_number == 1 else rf.workspace().project("damage-type-nogzj")
    model = project.version(1).model if model_number == 1 else project.version(2).model
...
def draw_annotations(image_path, predictions):
    ...
    for prediction1 in predictions['predictions']:
        ...
        cv2.putText(image, str(class_name)+" "+str(confidence), (int(text_x), int(text_y)), font, font_scale, color, font_thickness)
...
def annotate_image():
    ...
    annotated_image,time_taken_models = run_models_parallel('input.jpg', 20, file_name)
